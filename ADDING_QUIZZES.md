...
どの形式にも共通で、任意の `image` フィールドを付けられる。問題文の下に画像を表示する。
```ts
{
  type: 'choice',
  question: '次の回路図で、R1に流れる電流はいくらか。',
  image: 'images/circuit-1.png', // publicフォルダからの相対パス、またはdata URI
  choices: ['1A', '2A', '3A', '4A'],
  answer: 1,
}
```
- 画像ファイルを手で追加する場合は `public/images/` などに置き、`image` にその相対パス(`index.html`から見た場所、例: `images/circuit-1.png`)を書く。
- クイズ作成ツール(`create.html`)を使う場合は、ファイルを選ぶだけで自動的にdata URI(`data:image/...`から始まる文字列)として問題データに埋め込まれる。ファイルを個別に配置する必要がなく、`.ts` 1つで完結する。大きい画像は自動で縮小・再圧縮される。
- data URIを手で書く場合、1つの画像で数百KB〜1MB程度になることがあるので、画像は事前に圧縮・縮小しておくとよい。
